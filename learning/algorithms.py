...
class AbstractTwoStage(Algorithm):
    def __init__(self, data_type, input_shape, num_classes, num_attributes, num_examples, hparams, grp_sizes=None, attr_sizes=None):
        ...
        self.stage1_model = ERM(data_type, input_shape, num_classes, num_attributes, num_examples, hparams, grp_sizes, attr_sizes)
...
class JTT(AbstractTwoStage):
    ...
    """
    def __init__(self, data_type, input_shape, num_classes, num_attributes, num_examples, hparams, grp_sizes=None, attr_sizes=None):
        super().__init__(data_type, input_shape, num_classes, num_attributes, num_examples, hparams, grp_sizes, attr_sizes)
...
class MA(ERM):
    """ERM with Moving Average (MA) prediction model (https://arxiv.org/abs/2110.10832)"""
    def __init__(self, data_type, input_shape, num_classes, num_attributes, num_examples, hparams, grp_sizes=None, attr_sizes=None):
        ...
        self.global_iter = 0
        self.ma_count = 0
        self.ma_start_iter = hparams["ma_start_iter"]
...
class SAM(ERM):
    ...
    def update(self, minibatch, step):
        ...
        # 1. eps(w) = rho * g(w) / g(w).norm(2) = (rho / g(w).norm(2)) * g(w)
        grad_w = autograd.grad(loss, self.network.parameters())
        scale = self.hparams["sam_rho"] / self.norm(grad_w)
